export default function About() {
  return (
    <div className="px-4 md:px-8 lg:px-16">
      <h1 className="text-2xl md:text-3xl font-bold text-center text-white drop-shadow mb-8 md:mb-10">
        Welcome to niyu web page!!
      </h1>

      <section className="mb-8 md:mb-10">
        <h2 className="text-lg md:text-xl text-white mb-4">Introduction</h2>
        <ul className="space-y-2 text-base md:text-lg text-white pl-4">
          <li>所属大学：広島市立大学</li>
          <li>学部・学科：情報科学部 情報科学科 コミュニケーション講座</li>
          <li>所属サークル：いちぴろエクスプローラ，天地創造部</li>
          <li>興味・志望分野：ネットワーク、セキュリティ、アプリ開発</li>
          <li>所属団体：特定非営利活動法人 NxTEND イベント事業部本部 中国・四国事業部</li>
        </ul>
      </section>

      <section className="mb-8 md:mb-10">
        <h2 className="text-lg md:text-xl text-white mb-4">Work History</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white text-black rounded-xl shadow p-4">
            <span className="inline-block bg-orange-400 text-white text-xs px-2 py-1 rounded-full mb-2">アルバイト</span>
            <h3 className="font-bold">スタートプログラミング</h3>
            <p className="text-sm md:text-base">期間：2023年10月〜現在</p>
            <p className="text-sm mt-2">
              プログラミングスクールで講師のアルバイトをしています。
            </p>
          </div>

          <div className="bg-white text-black rounded-xl shadow p-4">
            <span className="inline-block bg-orange-400 text-white text-xs px-2 py-1 rounded-full mb-2">業務委託</span>
            <h3 className="font-bold">業務委託</h3>
            <p className="text-sm md:text-base">期間：2025年06月〜現在</p>
            <p className="text-sm mt-2">
              スタートプログラミングの業務委託として、プログラミングスクールの教材開発に携わっています。
            </p>
          </div>

          <div className="bg-white text-black rounded-xl shadow p-4">
            <span className="inline-block bg-orange-400 text-white text-xs px-2 py-1 rounded-full mb-2">業務委託</span>
            <h3 className="font-bold">業務委託</h3>
            <p className="text-sm md:text-base">期間：2025年02月〜現在</p>
            <p className="text-sm mt-2">
              Androidアプリの開発に携わっています。主にKotlinとJetpack Composeを用いてUIを開発しています。
            </p>
          </div>
        </div>
      </section>

      <section className="mb-8 md:mb-10">
  <h2 className="text-lg md:text-xl text-white mb-4">Events</h2>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2023.11.25</p>
    <p className="font-bold">セキュリティ・ミニキャンプ in 広島 2023</p>
    <p className="text-sm">広島で開催されたセキュリティの勉強イベントに参加し、実践的な講義や演習を体験しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2024.02.03</p>
    <p className="font-bold">塩尻サイバーセキュリティ勉強会2024年冬 in 塩尻</p>
    <p className="text-sm">塩尻で開催された勉強会で、セキュリティに関する最新の話題を学びました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.03.15</p>
    <p className="font-bold">P3NFEST 2025 Winter</p>
    <p className="text-sm">学生向けのセキュリティカンファレンスに参加し、多くの刺激を受けました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.03.16</p>
    <p className="font-bold">第2回 セキュリティ若手の会（LT&交流会）</p>
    <p className="text-sm">セキュリティに関心のある若手同士でLTや交流を行いました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.03.22</p>
    <p className="font-bold">Setory</p>
    <p className="text-sm">セキュリティとストーリーをテーマにしたユニークなイベントに参加しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">登壇</span>
    <p className="text-sm md:text-base">2025.05.03</p>
    <p className="font-bold">第2回広島ITイノベーションフェスタ</p>
    <p className="text-sm">イベントで登壇し、自身の学びや取り組みを発表しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.05.29</p>
    <p className="font-bold">のみぞうさんと2時間でハッキングをはじめよう</p>
    <p className="text-sm">初心者向けハッキング講座に参加し、基礎的な技術を体験しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.06.07</p>
    <p className="font-bold">Mini P3NFEST</p>
    <p className="text-sm">学生向けセキュリティカンファレンスに参加し、さまざまな知識を深めました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.06.26</p>
    <p className="font-bold">NFLabs. Purple Flair Workshop #1</p>
    <p className="text-sm">ワークショップ形式で実践的なサイバーセキュリティを学びました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">参加</span>
    <p className="text-sm md:text-base">2025.10.25</p>
    <p className="font-bold">セキュリティミニキャンプ in Hiroshima 2025</p>
    <p className="text-sm">広島で開催されたセキュリティの勉強イベントに参加し、実践的な講義や演習を体験しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-yellow-400 text-white text-xs px-2 py-1 rounded-full mb-2">運営</span>
    <p className="text-sm md:text-base">2025.12.06</p>
    <p className="font-bold">Setory vol.3</p>
    <p className="text-sm">広島で開催されたSetory vol.3に運営として参加しました。</p>
  </div>

</section>

<section className="mb-8 md:mb-10">
  <h2 className="text-lg md:text-xl text-white mb-4">Internship</h2>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.05.31（土）</p>
    <p className="font-bold">Docomo</p>
    <p className="text-sm">
      ドコモグループのセキュリティ業務と、フィッシングサイトの仕組みについて学びました。
    </p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.07.05（土）</p>
    <p className="font-bold">CyberAgent</p>
    <p className="text-sm">
      データセンタの見学や実際にサーバーやスイッチを触ってネットワークについて学びました。
    </p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.08.16（月）~ 08.29（金）</p>
    <p className="font-bold">チームラボ株式会社</p>
    <p className="text-sm">
      スマホアプリエンジニアとしてサマーインターンに参加させていただきました。
    </p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.08.30（土）</p>
    <p className="font-bold">株式会社ジーニー</p>
    <p className="text-sm">
      Webアプリケーションの実装課題に取り組みました。
    </p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">仕事体験</span>
    <p className="text-sm md:text-base">2025.09.04（木）~ 2025.09.05（金）</p>
    <p className="font-bold">株式会社エネコム</p>
    <p className="text-sm">
      二日間でネットワーク部門とシステム部門の仕事体験をしました。
    </p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.09.17（水）</p>
    <p className="font-bold">株式会社インターネットイニシアティブ</p>
    <p className="text-sm">オンラインのセキュリティ概論に参加しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.10.27（月）</p>
    <p className="font-bold">株式会社インターネットイニシアティブ</p>
    <p className="text-sm">SOCの見学とエンジニアの方と交流しました。</p>
  </div>

  <div className="bg-white text-black p-4 rounded-xl shadow mb-4">
    <span className="inline-block bg-blue-500 text-white text-xs px-2 py-1 rounded-full mb-2">インターン</span>
    <p className="text-sm md:text-base">2025.11.09（日）</p>
    <p className="font-bold">株式会社NTT ME</p>
    <p className="text-sm">
      インフラの議場についてゲーム形式で学びました。
    </p>
  </div>

</section>
    </div>
  );
}